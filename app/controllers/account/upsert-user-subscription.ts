...
import { supabaseAdminClient } from '@/config/supabse/supabase-admin';
import { stripeAdmin } from '@/config/stripe/stripe-admin';
import { Database } from '@/config/supabse/type';
import { toDateTime } from '@/lib/to-date-time';
...
export async function upsertUserSubscription({
    ...
    const subscriptionData: Database['public']['Tables']['subscriptions']['Insert'] = {
        ...
        current_period_start: toDateTime(Date.now()).toISOString(),
        current_period_end: subscription.ended_at ? toDateTime(subscription.ended_at).toISOString() : "N/A",
